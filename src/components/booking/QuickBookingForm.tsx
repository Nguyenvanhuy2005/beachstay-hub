...
const QuickBookingForm: React.FC<QuickBookingFormProps> = ({ onFullBooking }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name || !email || !phone || !roomType || !date) {
      toast.error(
        isVietnamese 
          ? 'Vui lòng điền đầy đủ thông tin đặt phòng!' 
    ...
    toast.success(
      isVietnamese 
        ? 'Yêu cầu đặt phòng đã được gửi thành công!' 
        : 'Booking request has been sent successfully!'
    );
  ...
  return (
    <div className="bg-white rounded-lg shadow-xl p-6 md:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button 
          type="submit" 
          className="w-full bg-beach-600 hover:bg-beach-700 py-6"
        >
      ...
      {onFullBooking && (
        <div className="mt-4 text-center">
          <button 
            onClick={onFullBooking}
            className="text-beach-700 hover:text-beach-800 underline text-sm"
          >
