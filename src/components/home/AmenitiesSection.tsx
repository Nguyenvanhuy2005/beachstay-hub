    ...
    const fetchAmenities = async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('amenities').select('*').order('id', {
          ascending: true
        });
        if (error) {
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-700 max-w-3xl mx-auto">
            {language === 'vi' ? 'Chúng tôi cung cấp đầy đủ tiện ích và dịch vụ chất lượng cao, đảm bảo kỳ nghỉ của bạn luôn thoải mái và thư giãn.' : 'We provide comprehensive amenities and high-quality services, ensuring your vacation is always comfortable and relaxing.'}
          </p>
        </div>
        ...
        {loading ? <div className="flex justify-center items-center py-20">
          ...
          return <motion.div key={amenity.id} className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow" variants={itemVariants} whileHover={{
            y: -5,
            transition: {
              duration: 0.2
            }
