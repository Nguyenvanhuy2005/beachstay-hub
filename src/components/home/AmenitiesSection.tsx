...
import { Bath, Utensils, Wifi, Coffee, Bike, FlameKindling, Map, LayoutGrid } from "lucide-react";
...
const AmenitiesSection = () => {
  ...
  useEffect(() => {
    const fetchAmenities = async () => {
      try {
        setLoading(true);
        const { data, error } = await supabase.from('amenities').select('*').order('id', { ascending: true });
