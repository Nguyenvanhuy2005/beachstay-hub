  ...
  const searchAvailableRooms = async (filters: SearchFilters) => {
    try {
      ...
      for (const room of roomTypes) {
        ...
        if (available) {
          availableRooms.push({ ...room, remainingRooms });
        }
      }
    ...
    <MainLayout>
      <div className="relative bg-beach-700 text-white">
        ...
        <div className="relative z-20 container mx-auto px-4 py-20 md:py-32">
          ...
          >
            {language === 'vi' 
              ? 'Khám phá các lựa chọn phòng nghỉ sang trọng tại An Nam Village, được thiết kế để đáp ứng mọi nhu cầu của bạn.' 
              : 'Explore our luxurious accommodation options at An Nam Village, designed to meet all your needs.'}
          </motion.p>
        </div>
      </div>
                          ...
                          {room.weekend_price && room.weekend_price !== room.price && (
                            <span className="text-gray-500 text-xs block mt-1">
                              {language === 'vi' 
                                ? isWeekend() 
                                  ? 'Giá cuối tuần' 
                                  : 'Giá ngày thường: ' + formatPrice(room.price) + 'đ' 
                                : isWeekend() 
                                  ? 'Weekend price' 
                                  : 'Regular price: ' + formatPrice(room.price) + 'đ'}
                            </span>
                          )}
                        </div>
                      </div>
                      <div className="text-beach-700 mb-4 flex-grow">
                        <p>{truncateDescription(getRoomDescription(room), room.id)}</p>
                        {getRoomDescription(room).length > 150 && (
                          <button 
                            onClick={() => toggleDescription(room.id)} 
                            className="text-beach-600 font-medium mt-2 text-sm flex items-center hover:underline"
                          >
                            {expandedDescriptions[room.id]
                              ? <>
                                  {language === 'vi' ? 'Thu gọn' : 'Read less'}
